...
from mobilenet_rm_filt_pt import MobileNetv1
...
def train_and_evaluate_kd(model, teacher_model, train_dataloader, val_dataloader, optimizer,
    ...
    # for cnn models, num_epoch is always < 100, so it's intentionally not using scheduler here
        ...
        scheduler = StepLR(optimizer, step_size=100, gamma=0.2) 
    elif params.model_version == "mobilenetv1_distill": 
    ...
    for epoch in range(params.num_epochs):
        ...
        if is_best:
            ...
            # Save the best model
            torch.save(model, "optimum0.96_pruned_KD.pt")
            # torch.save(model.state_dict(), 'trained_mobilenetv1_KD.pt')
...
if __name__ == '__main__':
    ...
    if "distill" in params.model_version:
        ...
        if params.model_version == "cnn_distill":
            ...
            # model = mobilenetv1.MobileNetv1().cuda() if params.cuda else mobilenetv1.MobileNetv1()



            model = torch.load("optimum0.96_pruned.pt").cuda() if params.cuda else torch.load("optimum0.96_pruned.pt")
            ...
            optimizer = optim.Adam(model.parameters(), lr=params.learning_rate)
            # fetch loss function and metrics definition in model files
            loss_fn_kd = mobilenetv1.loss_fn_kd
            metrics = mobilenetv1.metrics
