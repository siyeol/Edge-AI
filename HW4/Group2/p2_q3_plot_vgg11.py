...
df_rsp = open("./rsp/log_raspberry_vgg11.csv")
df_mc1 = open("./mc1/log_mc1_vgg11.csv")
...
for line in df_rsp:
    ...
    xtime = float(time_stamp) - 1648517339.3162882
    times_rsp.append(float(xtime))
    powers_rsp.append(float(total_power))
    temps_rsp.append(float(temps_avg))
    if float(time_stamp) >= 1648517339.9171257 and float(time_stamp) <= 1648519506.9401393:
        runpow_rsp.append(float(total_power))

for line in df_mc1:
    (time_stamp, total_power, temps_avg, temp) = line.split(',')
    xtime = float(time_stamp) - 1648582501.050831
    times_mc1.append(float(xtime))
    powers_mc1.append(float(total_power))
    temps_mc1.append(float(temps_avg))
    if float(time_stamp) >= 1648582501.8518972 and float(time_stamp) <= 1648583406.3017142:
