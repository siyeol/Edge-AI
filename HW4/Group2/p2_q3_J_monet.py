...
df_rsp = open("./rsp/log_raspberry_monet.csv")
df_mc1 = open("./mc1/log_mc1_monet.csv")
...
for line in df_rsp:
    ...
    xtime = float(time_stamp) - 1648575085.7922513
    times_rsp.append(float(xtime))
    powers_rsp.append(float(total_power))
    temps_rsp.append(float(temps_avg))
    if float(time_stamp) >= 1648575086.5934422 and float(time_stamp) <= 1648576055.3751533:
        runpow_rsp.append(float(total_power))

for line in df_mc1:
    (time_stamp, total_power, temps_avg, temp) = line.split(',')
    xtime = float(time_stamp) - 1648599445.8640108
    times_mc1.append(float(xtime))
    powers_mc1.append(float(total_power))
    temps_mc1.append(float(temps_avg))
    if float(time_stamp) >= 1648599448.1577287 and float(time_stamp) <= 1648600150.0078673:
...
plt.legend()
plt.grid()
plt.savefig("power_plot_monet.png")
...
plt.legend()
plt.grid()
plt.savefig("temp_plot_monet.png")
