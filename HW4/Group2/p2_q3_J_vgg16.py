...
df_rsp = open("./rsp/log_raspberry_vgg16.csv")
df_mc1 = open("./mc1/log_mc1_vgg16.csv")
...
for line in df_rsp:
    ...
    xtime = float(time_stamp) - 1648520059.5333123
    times_rsp.append(float(xtime))
    powers_rsp.append(float(total_power))
    temps_rsp.append(float(temps_avg))
    if float(time_stamp) >= 1648520059.9338818 and float(time_stamp) <= 1648523637.1372702:
        runpow_rsp.append(float(total_power))

for line in df_mc1:
    (time_stamp, total_power, temps_avg, temp) = line.split(',')
    xtime = float(time_stamp) - 1648583492.54235
    times_mc1.append(float(xtime))
    powers_mc1.append(float(total_power))
    temps_mc1.append(float(temps_avg))
    if float(time_stamp) >= 1648583495.8376794 and float(time_stamp) <= 1648585027.261519:
